...
import {MAT_DIALOG_DATA, Dialog, MatDialogModule, DialogRef} from './index';
...
describe('Dialog', () => {
  ...
  it('should open a dialog with a component', () => {
    ...
    viewContainerFixture.detectChanges();
    let dialogContainerElement = overlayContainerElement.querySelector('dialog-container')!;
  ...
  it('should close a dialog and get back a result', async(() => {
    ...
    viewContainerFixture.whenStable().then(() => {
      ...
      expect(overlayContainerElement.querySelector('dialog-container')).toBeNull();
    });
  }));
  ...
  it('should close a dialog and get back a result before it is closed', async(() => {
    ...
    const beforeCloseHandler = jasmine.createSpy('beforeClose callback').and.callFake(() => {
      expect(overlayContainerElement.querySelector('dialog-container'))
  ...
  it('should close from a ViewContainerRef with OnPush change detection', fakeAsync(() => {
    ...
    expect(overlayContainerElement.querySelectorAll('dialog-container').length)
  ...
  it('should close when clicking on the overlay backdrop', async(() => {
    ...
    viewContainerFixture.whenStable().then(() => {
      expect(overlayContainerElement.querySelector('dialog-container')).toBeFalsy();
    });
  }));
  ...
  it('should emit the keyboardEvent stream when key events target the overlay', async(() => {
    ...
    let backdrop = overlayContainerElement.querySelector('.cdk-overlay-backdrop') as HTMLElement;
    let container = overlayContainerElement.querySelector('dialog-container') as HTMLElement;
  ...
  it('should close all of the dialogs', async(() => {
    ...
    expect(overlayContainerElement.querySelectorAll('dialog-container').length).toBe(3);

    dialog.closeAll();
    viewContainerFixture.detectChanges();

    viewContainerFixture.whenStable().then(() => {
      expect(overlayContainerElement.querySelectorAll('dialog-container').length).toBe(0);
    });
  }));
  ...
  it('should set the proper animation states', () => {
    ...
    let dialogContainer: DialogContainer =
        viewContainerFixture.debugElement.query(By.directive(DialogContainer)).componentInstance;
  ...
  it('should close all dialogs when the user goes forwards/backwards in history', async(() => {
    ...
    expect(overlayContainerElement.querySelectorAll('dialog-container').length).toBe(2);
  ...
  describe('disableClose option', () => {
    it('should prevent closing via clicks on the backdrop', () => {
      ...
      expect(overlayContainerElement.querySelector('dialog-container')).toBeTruthy();
    });
  ...
  describe('dialog content elements', () => {
    let dialogRef: DialogRef<ContentElementDialog>;
    ...
    it('should close the dialog when clicking on the close button', async(() => {
      expect(overlayContainerElement.querySelectorAll('.dialog-container').length).toBe(1);

      (overlayContainerElement.querySelector('button[dialog-close]') as HTMLElement).click();
      viewContainerFixture.detectChanges();
    ...
    it('should not close the dialog if [dialog-close] is applied on a non-button node', () => {
      expect(overlayContainerElement.querySelectorAll('.dialog-container').length).toBe(1);

      (overlayContainerElement.querySelector('div[dialog-close]') as HTMLElement).click();

      expect(overlayContainerElement.querySelectorAll('.dialog-container').length).toBe(1);
    });

    it('should allow for a user-specified aria-label on the close button', async(() => {
      let button = overlayContainerElement.querySelector('button[dialog-close]')!;
    ...
    it('should return the [dialog-close] result when clicking the close button', async(() => {
      let afterCloseCallback = jasmine.createSpy('afterClose callback');
      dialogRef.afterClosed().subscribe(afterCloseCallback);
    ...
    it('should set the aria-labelledby attribute to the id of the title', async(() => {
      let title = overlayContainerElement.querySelector('[dialog-title]')!;
      let container = overlayContainerElement.querySelector('dialog-container')!;
...
@Component({
  template: `
    <h1 dialog-title>This is the title</h1>
    <dialog-content>Lorem ipsum dolor sit amet.</dialog-content>
    <dialog-actions>
      <button dialog-close [aria-label]="closeButtonAriaLabel">Close</button>
      <button class="close-with-true" [dialog-close]="true">Close and return true</button>
      <div dialog-close>Should not close</div>
    </dialog-actions>
  `
})
